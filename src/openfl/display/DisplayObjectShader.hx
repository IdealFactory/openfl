...
class DisplayObjectShader extends Shader
{
	...
	@:glFragmentHeader("in float openfl_Alphav;
		...
		uniform sampler2D openfl_Texture;
		uniform vec2 openfl_TextureSize;")
	@:glFragmentBody("vec4 color = texture (openfl_Texture, openfl_TextureCoordv);
		...
		if (color.a == 0.0) {
			...
			if (color.a > 0.0) {
				...
				fragColor = vec4 (color.rgb * color.a * openfl_Alphav, color.a * openfl_Alphav);
