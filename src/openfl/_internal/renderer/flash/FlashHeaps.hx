...
class FlashHeaps
{
	public static function render(heaps:HeapsContainer):Void
	{
		if (heaps.__appClass != null && heaps.__bitmapData != null)
		{
			...
			var pt = heaps.localToGlobal(new Point(heaps.x, heaps.y));

			var raw = heaps.__transform.rawData;
			raw[0] = heaps.transform.concatenatedMatrix.a;
			raw[4] = heaps.transform.concatenatedMatrix.b;
			raw[1] = heaps.transform.concatenatedMatrix.c;
			raw[5] = heaps.transform.concatenatedMatrix.d;
			raw[12] = 0; //-0.5; // ((heaps.width * 0.5) - heaps.transform.concatenatedMatrix.tx) / st.stageWidth;
			raw[13] = 0; //-0.5; // ((-heaps.height * 0.5) + heaps.transform.concatenatedMatrix.ty) / st.stageHeight;
			heaps.__transform.rawData = raw;
			heaps.__transform.prependTranslation(1, -1, 0);
			heaps.__transform.appendTranslation(-1, 1, 0);

			var stage3d = Lib.current.stage.stage3Ds[0];
			// trace("Position: s.x=" + stage3d.x + " h.x=" + heaps.x + " pt.x=" + pt.x + " concatd:" + heaps.transform.concatenatedMatrix.tx);
			// trace("Position: s.y=" + stage3d.y + " h.y=" + heaps.y + " pt.y=" + pt.y + " concatd:" + heaps.transform.concatenatedMatrix.ty);
			// trace("wid/hgt: stage=" + st.stageWidth + "/" + st.stageHeight + " h=" + heaps.width + "/" + heaps.height);
			// trace("scale: stage=" + heaps.scaleX + "/" + heaps.scaleY);
			// trace("matrix-concatenated:" + heaps.transform.concatenatedMatrix);
			// trace("raw:" + raw);
